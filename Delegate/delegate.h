...
private:
	...
	{
		auto a = (Delegate<Ret, Args...>*)0x7fffffffffff;
		auto addr = (Ret(*)(void*, Args...))0x7ffffffffffe;
		return addr(a, args...);
	}

	__declspec(noinline) static Ret forward2(Delegate* del, Args... args)
	{
		return del->operator()(args...);
	}
#endif
private:
	void Init()
...
public:
...
#else
	Delegate(Delegate&& ref) noexcept : invoker(ref.invoker)
	{
		ref.invoker = nullptr;
	}
#endif
	...
	//若要使用此函数，请确保满足以下条件
	//1、目标平台为Windows x64
	//2、使用MSVC编译器 (Visual Studio 或 MSBuild Tools)
	//3、若为Debug模式，请禁用仅我的代码调试
	Func GetFuncPtr()
	{
